...
<head>
    ...
    <title>Panel de Control - CRM Marcloi</title>
...
<body class="bg-zinc-950 min-h-screen text-zinc-100 antialiased">
    ...
    <main class="sm:ml-64 min-h-screen p-8 lg:p-12" id="main-content">
        ...
        <header class="flex flex-col md:flex-row md:items-center justify-between gap-6 pb-8 border-b border-zinc-900 mb-8">
            <div>
                <h1 class="text-3xl font-bold text-zinc-100 tracking-tight">Resumen General</h1>
                <p class="text-[14px] text-zinc-400 mt-1 font-medium">Métricas de negocio en tiempo real y actividad reciente.</p>
            </div>
            <div class="flex items-center gap-4">
                <button onclick="location.reload()" aria-label="Actualizar Panel" class="p-2.5 bg-zinc-900 border border-zinc-800 rounded-md text-zinc-400 hover:text-zinc-100 hover:bg-zinc-800 transition-colors focus-visible:outline focus-visible:outline-2 focus-visible:outline-indigo-500">
        ...
        <section aria-labelledby="metrics-heading" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-12">
            <h2 id="metrics-heading" class="sr-only">Indicadores Clave de Rendimiento</h2>
            
            <article class="bg-zinc-900 border border-zinc-800 p-6 rounded-xl flex flex-col justify-between">
                <div class="flex items-center justify-between mb-4">
                    <h3 class="text-[14px] font-semibold text-zinc-400">Clientes Totales</h3>
                    <i data-lucide="users" class="w-4 h-4 text-zinc-500" aria-hidden="true"></i>
                </div>
                <p class="text-3xl font-bold text-zinc-100" aria-label="<?php echo $totalLeads; ?> Clientes Totales"><?php echo number_format($totalLeads); ?></p>
            </article>

            <article class="bg-zinc-900 border border-zinc-800 p-6 rounded-xl flex flex-col justify-between">
                <div class="flex items-center justify-between mb-4">
                    <h3 class="text-[14px] font-semibold text-zinc-400">Ingresos Proyectados</h3>
                    <i data-lucide="dollar-sign" class="w-4 h-4 text-zinc-500" aria-hidden="true"></i>
                </div>
                <p class="text-3xl font-bold text-zinc-100" aria-label="<?php echo number_format($revenue, 2, ',', '.'); ?> Euros en Ingresos Proyectados">€<?php echo number_format($revenue, 2, '.', ','); ?></p>
            </article>

            <article class="bg-zinc-900 border border-zinc-800 p-6 rounded-xl flex flex-col justify-between">
                <div class="flex items-center justify-between mb-4">
                    <h3 class="text-[14px] font-semibold text-zinc-400">Captación de Pago</h3>
                    <i data-lucide="target" class="w-4 h-4 text-zinc-500" aria-hidden="true"></i>
                </div>
                <p class="text-3xl font-bold text-zinc-100" aria-label="<?php echo $sources['pago'] ?? 0; ?> clientes de canales de pago"><?php echo $sources['pago'] ?? 0; ?></p>
            </article>

            <article class="bg-zinc-900 border border-zinc-800 p-6 rounded-xl flex flex-col justify-between">
                <div class="flex items-center justify-between mb-4">
                    <h3 class="text-[14px] font-semibold text-zinc-400">Captación Orgánica</h3>
                    <i data-lucide="globe" class="w-4 h-4 text-zinc-500" aria-hidden="true"></i>
                </div>
                <p class="text-3xl font-bold text-zinc-100" aria-label="<?php echo $sources['organico'] ?? 0; ?> clientes orgánicos"><?php echo $sources['organico'] ?? 0; ?></p>
            </article>
        </section>
        ...
        <section aria-labelledby="activity-heading" class="bg-zinc-900 border border-zinc-800 rounded-xl overflow-hidden">
            <div class="flex items-center justify-between p-6 border-b border-zinc-800">
                <h2 id="activity-heading" class="text-[16px] font-bold text-zinc-100">Actividad Reciente</h2>
                <a href="leads.php" class="text-[14px] font-medium text-zinc-400 hover:text-zinc-100 transition-colors focus-visible:outline focus-visible:outline-2 focus-visible:outline-indigo-500 rounded-sm px-1">Ver todo</a>
            </div>

            <div class="divide-y divide-zinc-800">
                <?php while($lead = $recentLeads->fetch_assoc()): ?>
                <a href="leads.php" aria-label="Ver detalles de <?php echo htmlspecialchars($lead['name']); ?>" class="flex flex-col sm:flex-row sm:items-center justify-between p-4 sm:p-6 hover:bg-zinc-800/50 transition-colors focus-visible:bg-zinc-800/50 focus-visible:outline focus-visible:outline-2 focus-visible:outline-inset focus-visible:outline-indigo-500 group">
                    <div class="flex items-center gap-4 mb-4 sm:mb-0">
                        ...
                        <div class="flex flex-col">
                            ...
                            <span class="text-[14px] text-zinc-500"><?php echo htmlspecialchars($lead['company'] ?: 'Particular'); ?></span>
                        </div>
                    </div>
                    <div class="flex items-center gap-6 sm:gap-8 justify-between sm:justify-end w-full sm:w-auto">
                        <div class="flex flex-col sm:items-end">
                            <span class="text-[14px] font-bold text-zinc-200" aria-label="Cantidad: <?php echo number_format($lead['proposal_price'] ?? 0, 0, ',', '.'); ?> Euros">€<?php echo number_format($lead['proposal_price'] ?? 0, 0, '.', ','); ?></span>
                ...
                <?php if ($recentLeads->num_rows === 0): ?>
                    <div class="p-8 text-center">
                        <p class="text-[14px] text-zinc-500">No hay actividad reciente.</p>
                    </div>
                <?php endif; ?>
            </div>
        </section>
