...
<aside class="fixed left-0 top-0 z-40 w-64 h-screen transition-transform -translate-x-full sm:translate-x-0 bg-bg border-r border-border">
   <div class="h-full px-6 py-10 overflow-y-auto flex flex-col">
      ...
      <ul class="space-y-1 font-medium flex-1">
         <li>
            <a href="index" class="flex items-center px-4 py-3 text-zinc-400 rounded-xl hover:bg-zinc-900/50 hover:text-white transition-all <?php echo basename($_SERVER['PHP_SELF']) == 'index.php' ? 'bg-zinc-900 text-white' : ''; ?>">
               <i data-lucide="layout-dashboard" class="w-4 h-4"></i>
               <span class="ms-3 text-sm">Dashboard</span>
            </a>
         </li>
         <li>
            <a href="leads" class="flex items-center px-4 py-3 text-zinc-400 rounded-xl hover:bg-zinc-900/50 hover:text-white transition-all <?php echo basename($_SERVER['PHP_SELF']) == 'leads.php' ? 'bg-zinc-900 text-white' : ''; ?>">
               <i data-lucide="layers" class="w-4 h-4"></i>
               <span class="ms-3 text-sm">Leads</span>
            </a>
         </li>
         <li>
            <a href="explorer" class="flex items-center px-4 py-3 text-zinc-400 rounded-xl hover:bg-zinc-900/50 hover:text-white transition-all <?php echo basename($_SERVER['PHP_SELF']) == 'explorer.php' ? 'bg-zinc-900 text-white' : ''; ?>">
