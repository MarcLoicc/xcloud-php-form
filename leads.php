...
<body class="bg-dark text-white font-sans">
    ...
    <main class="sm:ml-64 p-6 sm:p-12 min-h-screen">
        <div class="mb-10 flex flex-col md:flex-row md:items-end justify-between gap-6">
            ...
            <!-- Trigger Modal -->
            <button onclick="toggleModal()" class="px-6 py-3 bg-indigo-600 hover:bg-indigo-500 text-white font-bold rounded-2xl flex items-center gap-2 transition-all transform hover:-translate-y-1 shadow-lg shadow-indigo-600/20">
            ...
            <div class="overflow-x-auto">
                    ...
                    <tbody class="divide-y divide-dark-border">
                        ...
                        <tr class="hover:bg-indigo-500/5 transition-colors group">
                            <td class="px-6 py-4 text-sm text-gray-500 font-medium">
                                ...
                                <span class="block text-[10px] opacity-50 uppercase"><?php echo date('H:i', strtotime($row['created_at'])); ?></span>
                            </td>
                            <td class="px-6 py-4">
                            ...
                            <td class="px-6 py-4">
                                <p class="text-sm text-gray-400 italic max-w-xs truncate" title="<?php echo htmlspecialchars($row['message']); ?>">
                                    <?php echo $row['message'] ? '"'.htmlspecialchars($row['message']).'"' : '<span class="opacity-20">Sin notas adicionales</span>'; ?>
                                </p>
                            </td>
                        </tr>
                        <?php endwhile; ?>
                ...
                <?php if ($result->num_rows == 0): ?>
                    <div class="p-20 text-center">
                        ...
                        <p class="text-gray-500 max-w-xs mx-auto mb-8">Empieza a registrar prospectos para verlos aquí listados.</p>
                        <button onclick="toggleModal()" class="text-indigo-400 font-bold hover:text-indigo-300 transition-colors underline underline-offset-8">Añadir mi primer lead &rarr;</a>
                    </div>
                <?php endif; ?>
            </div>
        </div>
